...
$(document).ready(function () {
    $("#searchInput").on("input", function () {
        ...
        $(".office-row").each(function () {
            var id = $(this).data("id").toString();
            var address = $(this).find("td:eq(1)").text().toLowerCase(); // Изменено на соответствующий индекс колонки
            var phone = $(this).find("td:eq(3)").text().toLowerCase(); // Изменено на соответствующий индекс колонки
            var hidden = $(this).hasClass("hidden-row");

            if ((id.includes(searchTerm) || address.includes(searchTerm) || phone.includes(searchTerm)) && (!hidden || (hidden && searchTerm === "false"))) {
                $(this).show();
            } else {
                $(this).hide();
            }
    ...
    $('.Hidden-button').click(function () {
        ...
        $('.hidden-checkbox').each(function () {
            if ($(this).data('changed')) {
                ...
                $.ajax({
                    method: 'POST',
                    url: '/update_hidden_status_offices/',  // Замените на реальный URL
                    data: {
                        office_id: officeId,
                        is_hidden: isChecked
                    },
